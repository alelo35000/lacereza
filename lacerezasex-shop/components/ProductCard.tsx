...
import { generateProductDescription, suggestProductTitle, generateProductImage } from '../services/geminiService';
import { Trash2, Plus, X, ZoomIn, MessageCircle, Loader2, Sparkles, Upload } from './Icons';
...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const handleGenerateDesc = async () => {
      ...
      const newDesc = await generateProductDescription(product.name);
      updateProduct(categoryKey, product.id, 'description', newDesc);
      setIsGeneratingDesc(false);
  };

  const handleSuggestTitle = async () => {
      setIsGeneratingTitle(true);
      const newTitle = await suggestProductTitle(product.description);
      updateProduct(categoryKey, product.id, 'name', newTitle);
      setIsGeneratingTitle(false);
  };

  const handleGenerateImage = async () => {
    if (!product.name || !product.description) {
        showNotification('Product name and description are required to generate an image.', 'warning');
        return;
    }
    setIsGeneratingImage(true);
    const result = await generateProductImage(product.name, product.description);
    if (result.startsWith('data:image')) {
        updateProduct(categoryKey, product.id, 'image', result);
        showNotification('AI image generated successfully!', 'success');
    } else {
        showNotification(result, 'error'); // Show error message from service
    }
    ...
    return (
      <div className="bg-gradient-to-br from-gray-800/80 to-black/80 backdrop-blur-lg rounded-lg shadow-2xl overflow-hidden border border-amber-500/50 p-6 flex flex-col gap-4">
        ...
        <div className="flex gap-2">
            ...
            />
             <button
                ...
                disabled={isGeneratingImage || !product.name || !product.description}
                className="bg-gradient-to-r from-purple-500 to-pink-500 text-white p-2 rounded cursor-pointer transition hover:scale-105 font-semibold flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <div className="relative w-full aspect-square bg-gray-900 rounded flex items-center justify-center overflow-hidden">
            ...
            {isGeneratingImage && (
                <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-white mt-4 text-sm font-semibold">Generating AI Image...</p>
        ...
        <div className="relative">
            ...
            <button onClick={handleSuggestTitle} disabled={isGeneratingTitle || !product.description} className="absolute right-2 top-2 text-purple-400 hover:text-purple-300 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <div className="relative">
          ...
          <button onClick={handleGenerateDesc} disabled={isGeneratingDesc || !product.name} className="absolute right-2 top-2 text-purple-400 hover:text-purple-300 disabled:opacity-50 disabled:cursor-not-allowed">
             {isGeneratingDesc ? <Loader2 size={20} className="animate-spin" /> : <Sparkles size={20} />}
          </button>
        </div>
